...
class PeftModel(PushToHubMixin, torch.nn.Module):
    ...
    **Attributes**:
    ...
    def get_prompt_parameters(self):
        return [val.cpu().numpy() for _, val in self.prompt_encoder.state_dict().items()]

    def set_prompt_parameters(self, params): 
        params_dict = zip(self.prompt_encoder.state_dict().keys(), params)
        state_dict = OrderedDict({k: torch.Tensor(v) for k, v in params_dict})
        self.prompt_encoder.load_state_dict(state_dict, strict = True)
